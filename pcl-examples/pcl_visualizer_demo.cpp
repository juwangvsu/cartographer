...
void show_again(pcl::PointCloud<pcl::PointXYZ>::ConstPtr cloud, pcl::visualization::PCLVisualizer::Ptr viewer)
{
...
main (int argc, char** argv)
{
  ...
  while (!viewer->wasStopped ())
  {
   ...
   show_again(basic_cloud_ptr, viewer);
  while (!viewer->wasStopped ())
  {
    viewer->spinOnce (100);
    std::this_thread::sleep_for(100ms);
  ...
  */
    ...
    std::cout<<"loop1 done " << viewer <<"\n";
    while (true) {
	    std::cout<<"dummy loop\n";
	    sleep(2);
    }
    viewer = simpleVis(basic_cloud_ptr);
    std::cout<<"2nd viewer " << viewer <<"\n";
  while (!viewer->wasStopped ())
  {
    viewer->spinOnce (100);
    std::this_thread::sleep_for(100ms);
    ...
    std::cout<<"2nd viewer done " << viewer;
}
